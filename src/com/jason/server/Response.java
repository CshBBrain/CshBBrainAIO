...
public class Response{
	...
	private int transCount = 0;// 当前buffer已经传输的自己 
	...
	public void write(Client sk){
		...
		if(this.currentBuffer != null){			
			sk.getSocketChannel().write(this.currentBuffer, this, sk.getWriteHandler());
		}else{
			if(this.parent != null){
				this.parent.parentRelease();
			}
			this.client.getInWrite().set(false);			
			this.client.writeMessage();// 继续回写其他响应数据
		}
		
		// 如果有文件则传输文件
		/*if(this.currentBuffer == null && !MyStringUtil.isBlank(this.filePath)){
		    try{
	...
	public void bufferedContent(){
		if(!this.isBuffered){// 如果没有做数据缓存处理则缓存数据
